...
Phase_t = dist.Field(name='Phase_t', bases=(zbasis, xbasis))
...
# Plot grid values
figkw = {'figsize': (10, 5), 'dpi': 100}
plt.figure(**figkw)
mappable = plt.imshow(Phase['g'], cmap=cm, origin='lower', extent=(0, Lx, 0, Lz))
# plot_bot_2d(Phase, figkw=figkw, title="f['g']")

plt.colorbar(mappable, label="Phase (1: Ice, 0: Water)")
plt.title("2D Melting Process")
plt.xlabel('x (cm)')
plt.ylabel('z (cm)')
plt.show()
...
snapshots.add_task(d3.Average(Phase, 'x'), name='Phase')  # new
snapshots.add_task(d3.Average(Phase_t, 'x'), name='Phase_t')  # new
